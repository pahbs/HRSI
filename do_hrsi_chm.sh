...
out_dir_main=/att/nobackup/pmontesa/hrsi_chm_work
...
final_chm_dir=/att/nobackup/pmontesa/hrsi_chm_20170813
...
for i in $pairlist; do
	ln -sf `echo ${in_dir}/${i}/${dem}` `echo ${in_dir}/${i}/${i}-${demtail}` | tee -a $logFile
	full_pairlist="$full_pairlist `echo ${in_dir}/${i}/${i}-${demtail}`"
done
...
out_dz=$(echo $pairlist | awk -v dem_var="${demtail%.*}" -F" " '{print $1"-"dem_var"_warp-trans_reference-DEM_"$2"-"dem_var"_warp-trans_reference-DEM"}')_dz_eul.tif
echo; echo $out_chm_dir/$out_dz; echo
