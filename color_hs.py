...
def main():
    ...
    if hs_overlay:
        ...
        if not os.path.exists(overlay_fn):
            ...
            #this will run into imagemagick memory limits even though plenty of memory seems available.
            ...
            try:
                ...
                print("\nMemory issue creating %s") %overlay_fn
                #cmd = "colormap {} -s {} -o {} --colormap-style {}".format(fn, hs_fn, overlay_fn, colormapFile)
                #cmd.extend(iolib.gdal_opt_co)
                #cmd.extend((overlay_fn, tmp_fn))
                #print(' '.join(cmd))
                #subprocess.call(cmd, shell=False)
