...
def main(args):
    ...
    if ext == '.gdb':
        outGpkg = outGdb.replace('.gdb', '.gpkg')
        convertToGdb = True
    elif ext == '.gpkg':
        outGpkg = outGdb
        convertToGdb = False
    ...
    badLon = [] # list of longitudes that are < 180
    ...
    with open(inCsv, 'r') as c:
        ...
        for row in data:
            ...
            # Some rows have a large longitude. Record info in list
            if float(rowDict['lon']) > 180:
            ...
            # Add some attributes to the dict:
            rowDict['uniqueID']  = cnt # Add unique ID to dict
